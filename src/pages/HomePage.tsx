...
export function HomePage({
  ...
  // Advanced topics come from the entitlement payload and carry no counts,
  // which lands in the same place: no extras, no control.
  const extraTopics = (topics?.categories ?? []).flatMap(c => c.topics)
    .filter(t => (t.extraCount ?? 0) > 0);
    ...
    <div className="min-h-screen bg-slate-900">
      <header className="bg-slate-800 border-b border-slate-700 sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-slate-100">Blood & Lymph QBank</h1>
            <p className="text-sm text-slate-400">Hematology & Lymphatics Question Bank</p>
          </div>
